...
| 파일 | 역할 | Git |
|---|---|---|
| `index.html` | 통합 리포트 + 프로토타입 (**6개 탭** — 01 프로토타입 · 02 화면설계 · 03 기능정의 · 04 요구사항 정의서 · 05 테스트 케이스 · 06 변경 이력) | 추적 |
...
| `REQUIREMENTS.md` | **C-01~C-66 전문(66건)** · 명세 원칙 · 추적성 매트릭스 · 표기 교정 현황 |
| `EVIDENCE.md` | 정량 근거 |
...
## index.html 내부 지도
> 2026-08-19 **04 개선안 + 05 요구사항 정의서 통합** — 탭 7 → **6개**. 줄 번호는 통합 직후 기준(총 5,323줄).

| section id | 탭 | 시작 줄 | 비고 |
|---|---|---:|---|
| `rt-proto` | 01 프로토타입 | 1208 | 엔진 리팩터링 금지 |
| `rt-func` | 03 기능정의 | 1263 | **판정 전용** — 화면·개발 범위 / 판정 시퀀스 / 예외 / 판정 데이터. 요소 명세·개선 근거는 참조만 |
| `rt-req` | 04 요구사항 정의서 | 1375 | **C-01 ~ C-66 전건 정의(66건)** · 8장(개선 배경 → 문제·원칙 → 프로세스 → 화면 → 데이터 → 공통 기준 → 범위 밖 → 문구·측정) |
| `rt-tc` | 05 테스트 케이스 | 1756 | **116건**(기능 26 · 예외 7 · 화면 45 · 레이어 17 · 접근성 13 · PC 8), 참조 전용 |
| `rt-log` | 06 변경 이력 | 1815 | 변경 이력 단일 출처 · 검색 + 더보기 |
| `rt-spec` | 02 화면설계 | 1875 | **주 작업 대상** · A ~ **H** 프레임 · 부록 0~6 · 파일 끝까지 |

> `rt-plan`(구 04 개선안)은 **제거**되었습니다. 그 내용은 `rt-req` 1 · 3 · 4 · 5 · 7장입니다.
